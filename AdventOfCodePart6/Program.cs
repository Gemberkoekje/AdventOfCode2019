...
namespace AdventOfCodePart5
{
    ...
    class Program
    {
        static string[] wire1 = new string[]
            {
        ...
        static string[] wire2 = new string[]
            {
        ...
        static List<Position> wire1Positions = new List<Position>();
        static List<Position> wire2Positions = new List<Position>();
        ...
        static void Main(string[] args)
        {
            ...
            Position curpos = new Position() { x = 0, y = 0, distance = 0 };
            wire1Positions.Add(new Position() { x = curpos.x, y = curpos.y, distance = curpos.distance});
            foreach(var knot in wire1)
            {
                var direction = knot[0];
                var l = knot.Substring(1);
            ...
            curpos = new Position() { x = 0, y = 0, distance = 0 };
            wire2Positions.Add(new Position() { x = curpos.x, y = curpos.y, distance = curpos.distance });
            foreach (var knot in wire2)
            {
                var direction = knot[0];
                var l = knot.Substring(1);
                var length = int.Parse(l);
            ...
            Console.WriteLine(string.Format("Lines to check: {0}", wire1Positions.Count));
            int t = wire1Positions.Count();

            foreach (var line in wire1Positions)
            {
                t--;
                if (t % 1000 == 1)
                {
                    Console.WriteLine(string.Format("Lines to check: {0}", t));
                }
                var q = wire2Positions.FirstOrDefault(w => w.x == line.x && w.y == line.y);
                if (q != null)
                {
                    if(line.x != 0 || line.y != 0)
                    {
                        var dist = Math.Abs(line.distance) + Math.Abs(q.distance);
                        if(closest > dist)
                        {
                            closest = dist;
                        }
