...
namespace AdventOfCodePart5
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Position curpos = new Position() { x = 0, y = 0 };
            wire1Positions.Add(new Position() { x = curpos.x, y = curpos.y});
            ...
            curpos = new Position() { x = 0, y = 0 };
            wire2Positions.Add(new Position() { x = curpos.x, y = curpos.y });
            ...
            {
                if(wire2Positions.Any(w => w.x == line.x && w.y == line.y))
                {
                    if(line.x != 0 || line.y != 0)
                    {
                        var dist = Math.Abs(line.x) + Math.Abs(line.y);
                        if(closest > dist)
                        {
                            closest = dist;
                        }
